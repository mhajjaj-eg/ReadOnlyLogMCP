...
1. `/` — Expected: JSON status payload.
2. `/mcp` — Expected: MCP HTTP endpoint for streamable HTTP clients.
3. `/mcp/sse` — Expected: legacy SSE endpoint if enabled.
4. `/downloads/log-bundle?...` — Expected: zip download for valid inputs.
